...
local function setupSpeedIndicator(char)
	...
	discordLabel.Text="discord.gg/Ghosthub";discordLabel.TextColor3=Color3.fromRGB(220,45,60)
...
local function saveConfig()
	...
	if writefile then pcall(function() writefile("ghostPC.json",HS:JSONEncode(cfg)) end) end
end
...
local function buildGui()
	...
	local REDDIM= Color3.fromRGB(180,180,180)
	...
	local old=game:GetService("CoreGui"):FindFirstChild("Ghost");if old then old:Destroy() end
	local pg=LP:FindFirstChild("PlayerGui");if pg then local o=pg:FindFirstChild("Ghost");if o then o:Destroy() end end
	local gui=Instance.new("ScreenGui")
	gui.Name="Ghost";gui.ResetOnSpawn=false;gui.DisplayOrder=10;gui.IgnoreGuiInset=true
	...
	ttl.BackgroundTransparency=1;ttl.Text="GHOST"
	...
	miniBtn.Text="GHOST";miniBtn.TextColor3=RED;miniBtn.Font=Enum.Font.GothamBold;miniBtn.TextSize=11
...
local function loadConfigKeys()
	if not(isfile and isfile("ghostPC.json")) then return end
	local ok,cfg=pcall(function() return HS:JSONDecode(readfile("ghostPC.json")) end)
